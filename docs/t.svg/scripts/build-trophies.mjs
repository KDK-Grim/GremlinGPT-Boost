...
// tuning (seconds)
const PAGE_SEC = Number(process.env.TROPHIES_PAGE_SEC || 6);  // slower, default 6s per page
const HOLD_FRAC = .75;                                        // % of time holding centered (0..1)
...
// page through owned repos to sum stars accurately
let starSum = 0;
let repoCount = 0;
let cursor = null;
do {
...
const d = await gql(`
  ...
  }`, { login: USER, from: fromISO, to: nowISO });
